...
void beforeKernelResidual(int mBlock, flow_var  & fl)
{
	...
	for (int i = 1; i <= mm1; ++i) {
		for (int j = 1; j <= nn1; ++j) {
			for (int k = 1; k <= 4; ++k) {
				U_host[i*(nn1+1)*5 + j*5 + k] = B.U[i][j][k];
			}
		}
	}
	...
	//HANDLE_ERROR(cudaDeviceSynchronize());
	//comput_duvtpckwC_with_cuda << <blockPerGrid, threadPerBlock >> > (transferInt_dev, transferDouble_dev, T, cc, U_dev);	//启动kernel 函数计算基本量
	HANDLE_ERROR(cudaDeviceSynchronize());
	...
	getResidual_i_WithKernel <<< blockPerGrid, threadPerBlock >>> ( transferInt_dev, transferDouble_dev,
								U_dev,Amu_dev, Amu_t_dev, x1_dev, y1_dev, x_dev, y_dev, Fluxi_dev, uu, vv, T);

	getResidual_j_WithKernel << < blockPerGrid, threadPerBlock >> > (transferInt_dev, transferDouble_dev,
	...
	HANDLE_ERROR(cudaMemcpy(Fluxi_host, Fluxi_dev, (nx + 1)*(ny + 1) * 5 * sizeof(double), cudaMemcpyDeviceToHost));
	double * Fluxj_host;
	HANDLE_ERROR(cudaHostAlloc((double **)&Fluxj_host, (nx + 1)*(ny + 1) * 5 * sizeof(double), cudaHostAllocDefault));
	HANDLE_ERROR(cudaMemcpy(Fluxj_host, Fluxj_dev, (nx + 1)*(ny + 1) * 5 * sizeof(double), cudaMemcpyDeviceToHost));
		...
		double * x1, double *y1, double *x, double *y, double *tFluxi, double *uu, double *vv, double * T)
{
	...
	if (i >= 1 && i <= nx && j >= 1 && j <= ny - 1) {
		...
		/*	if (i == 1 && j == 1) {
			...
			double put1= (t11*ni1 + t12*ni2)*si;
			double put2= (t12*ni1 + t22*ni2)*si;
			double put3= (E1*ni1 + E2*ni2)*si;
